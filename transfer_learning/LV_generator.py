...
@author: Ilya Timofeyev, Anton Erofeev
"""
...
for transfer learning: num_lg_steps = 1000 is enough
for testing (including histogram comparison): num_lg_steps = 5000000 at least

total train trajectory length = dt * num_sm_steps * num_lg_steps
"""
params = {
    "dim": 4, #LV dimension
    "dt": 0.01, # time step of integration
    "num_lg_steps" : 1000, # number of time steps Dt for output
    "num_lg_skip" : 1000, # skip this many large time-steps; start output after that
    "num_sm_steps": 400, # number of small time-steps in a big time-step
...
def init_condition(dim):
    ...
    #we add small random pertrubations for generating new trajectories
...
def compute_rhs(u):
    ...
    parameters 'r' and 'alf' are global
    d/dt u = r * u * (1 - alf *u)
    """
...
if __name__ == '__main__':
    ...
    """
    with open('test_trace_LV.pkl', 'wb') as file:
        pk.dump(usave.T, file)
    """
